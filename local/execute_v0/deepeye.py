...
class DeepEye:
    @profile
    def __init__(self, model_path, deep=2, layers=16):
        ...
        saver.restore(self.sess, str(pathlib.Path(__file__).parent) + model_path)
    ...
    @profile
    def dummy(self, prob_mask, session):
        for outer in prob_mask:
            for inner in outer:
                dumdum = inner
        return
    ...
    def run(self, frame):
        ...
        self.dummy(prob_mask, self.sess)
    ...
    @profile
    def processSingleImage(self, npimg):
        ...
        # PUPIL!!!
        (coords, bw_img) = self.run(frame_gray)
